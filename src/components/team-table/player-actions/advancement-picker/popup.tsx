...
export function Popup({ player, skills, onHide }: Props): React.ReactElement {
  ...
  const isCharacteristicSelectable = React.useCallback(
    (stat: "MA" | "AG" | "AV" | "PA" | "ST" | "Secondary") => {
      ...
      const inBounds =
        improvedStat > minStats[stat] && improvedStat < maxStats[stat];
  ...
  const [type, setType] = React.useState<keyof typeof advancementCosts>(() => {
    const opts = Object.keys(advancementCosts) as Array<
      keyof typeof advancementCosts
    >;
    const result = opts.find((key) =>
      advancementCosts[key].some((cost) => cost <= player.starPlayerPoints)
    );
    return result ?? "Random Primary";
  });
  const [randomCategory, setRandomCategory] =
    React.useState<SkillCategory | null>(
  ...
  const handleSkillSelection = React.useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const [statPrefs, setStatPrefs] = React.useState<
    Array<"MA" | "AG" | "ST" | "AV" | "PA" | "Secondary">
  >(() =>
    (["MA", "AG", "ST", "AV", "PA", "Secondary"] as const).filter((stat) =>
      isCharacteristicSelectable(stat)
    )
  );
  ...
  const handleUpgrade = (): void => {
    const update = ((): Parameters<
      typeof trpc.player.improve.mutate
    >[0]["update"] => {
      switch (type) {
        case "Characteristic Improvement": {
          if (selectedSkill === null) throw new Error("Skill not selected");
          return {
            type: "characteristic",
            preferences: statPrefs.slice(
              0,
              statPrefs.findIndex((val) => val === "Secondary")
            ) as [Exclude<(typeof statPrefs)[number], "Secondary">],
            skill: selectedSkill,
          };
        }
        case "Chosen Primary": {
        ...
        case "Random Primary": {
          if (randomCategory === null) throw new Error("Skill not selected");
          return {
            type: "random",
            subtype: "primary",
  ...
  return (
    <>
      <select value={type} onChange={handleTypeChange}>
        {Object.entries(advancementCosts).map(([adv, costs]) => (
          <option
            key={adv}
            value={adv}
            disabled={costs[player.totalImprovements] > player.starPlayerPoints}
          >
      ...
      {type.startsWith("Random") && (
        <React.Fragment key={type}>
          <select
            value={randomCategory ?? undefined}
          ...
          >
            {(getAvailableCategories(type) ?? []).map((category) => (
              <optgroup key={category} label={category}>
                {skills.filter(isSkillSelectable(category)).map((skill) => (
      ...
      {type === "Characteristic Improvement" && (
        <React.Fragment key={type}>
          <ol>
            {statPrefs.filter(isCharacteristicSelectable).map((c, idx, arr) => (
              <li key={`${c}-${idx}`}>
                {c}
                {c === "Secondary" && (
                  <select
                ...
                <span>
                  {idx !== 0 && (
                    <Button type="button" onClick={handleReorder("up", idx)}>
                      Up
                    </Button>
                  )}
                  {idx !== arr.length - 1 && (
                    <Button type="button" onClick={handleReorder("down", idx)}>
                      Down
                    </Button>
                  )}
                </span>
