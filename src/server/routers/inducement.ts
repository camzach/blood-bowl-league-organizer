...
export const inducementRouter = router({
  list: publicProcedure
    .input(z.object({ team: z.string() }).or(z.object({ specialRules: z.array(z.string()) })).or(z.undefined()))
      ...
      const specialRules = await (async(): Promise<string[] | null> => {
        if (typeof input === 'undefined') {
          ...
          const team = await ctx.prisma.team.findUniqueOrThrow({
            ...
            select: { roster: { select: { specialRules: { select: { name: true } } } } },
          });
        ...
        where: specialRules
          ? {
            OR: [
              ...
              { options: { some: { OR: [{ price: { not: null } }, { specialPriceRuleName: { in: specialRules } }] } } },
            ],
          }
          : undefined,
        include: {
