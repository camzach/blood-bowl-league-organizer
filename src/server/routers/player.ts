...
export const playerRouter = router({
  fire: publicProcedure.input(z.string()).mutation(async ({ input, ctx }) => {
  ...
  improve: publicProcedure
      ...
      z.object({
        ...
        update: z.discriminatedUnion("type", [
          ...
          z.object({
            ...
            subtype: z.literal("primary").or(z.literal("secondary")),
            category: z.enum(
              Object.values(SkillCategory) as [
                SkillCategory,
                ...SkillCategory[]
              ]
            ),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const skill =
        "skill" in update
          ? await ctx.prisma.skill.findUniqueOrThrow({
              where: { name: update.skill },
            })
          : null;
      ...
      if (update.type === "chosen" && skill !== null) {
        ...
        playerUpdate.teamValue = {
          increment: tvCostTable[`${update.type}${upperFirst(update.subtype)}`],
        };
        ...
        characteristicChoice =
          update.preferences.find((pref) => availableOptions.includes(pref)) ??
          "chosenSecondary";
      ...
      return ctx.prisma.$transaction(async (tx) => {
        ...
        const sppCost =
          update.type === "characteristic"
            ? sppCostTable[update.type][player.totalImprovements]
            : sppCostTable[`${update.type}${upperFirst(update.subtype)}`][
