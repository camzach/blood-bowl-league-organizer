...
export const gameRouter = router({
  start: publicProcedure
    ...
    .mutation(async ({ input: id, ctx }) => {
      ...
      ensureAuthenticationForTeams(ctx.session, [
        game.home.name,
        game.away.name,
      ]);
      ...
      return ctx.prisma
        .$transaction([teamUpdates, gameUpdate])
        .then(() => result);
    }),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const homeChoice =
        input.home !== undefined
          ? await ctx.prisma.position.findFirstOrThrow(
  ...
  purchaseInducements: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [homeWinnings, awayWinnings] = [
        ...
        (score) =>
          (score + (game.fanFactorHome + game.fanFactorAway) / 2) * 10_000
      );

      return ctx.prisma
        .$transaction([
          ...Object.values(updateMap).map((update) =>
            ctx.prisma.player.update(update)
          ),
          ctx.prisma.game.update({
            where: { id: game.id },
            data: {
