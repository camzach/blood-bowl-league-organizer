...
export type GameState = Array<z.infer<typeof gameEvent> & { eventId: string }>;
...
export default function ScoreWidget({ home, away, gameId }: Props) {
  ...
  const gameState: GameState = useMemo(() => {
    const param = searchParams.get("gameState");
    if (!param) return [];
    try {
  ...
  const setGameState = (updater: (old: GameState) => GameState) => {
    const newState = updater(gameState);
    const encoded = encodeURIComponent(btoa(JSON.stringify(newState)));
    setSearchParams({ gameState: encoded });
  };
  ...
  return (
    <div className="col-span-2 grid grid-cols-subgrid gap-y-3">
      ...
      <div className="ml-auto flex flex-col items-end gap-1 leading-10">
        ...
        {home.song && <audio src={`/songs/${home.id}`} ref={homeSongRef} />}
      </div>
      <div className="mr-auto flex flex-col items-start gap-1 leading-10">
        ...
        {away.song && <audio src={`/songs/${away.id}`} ref={awaySongRef} />}
      </div>
